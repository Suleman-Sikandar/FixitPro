...
<head>
    ...
    <style>
        ...
        .pac-item-query { font-size: 14px; color: #111827; font-weight: 700; }
        .pac-matched { color: #3b82f6; }
        ...
        .premium-swal-success-icon {
            border-color: #065f46 !important; /* Dark Green Circle */
            background-color: #065f46 !important;
        }
        .premium-swal-success-icon [class^='swal2-success-line'] {
            background-color: #ffffff !important; /* White Tick */
        }
...
<body class="bg-[#F8FAFC] min-h-screen flex flex-col antialiased selection:bg-blue-100 selection:text-blue-700" x-data="{ mobileMenu: false }">
    <!-- Navbar -->
    <nav class="glass-effect nav-glow sticky top-0 z-50">
            ...
            <div class="flex items-center justify-between h-20">
                        ...
                        @foreach($permittedMenu as $category)
                            <div class="relative" x-data="{ open: false }">
                                <button @click="open = !open" 
                                        :class="open ? 'nav-active' : 'nav-inactive'"
                                        class="nav-btn-base flex items-center gap-2">
                                    <i class="fas fa-cubes opacity-70"></i>
                                    {{ $category->name }}
                                    <i class="fas fa-chevron-down text-[10px] ml-1 transition-transform duration-300" :class="open ? 'rotate-180' : ''"></i>
                                </button>
                                ...
                                <div x-show="open" 
                                    ...
                                    <div class="px-4 py-2 mb-2 border-b border-gray-50">
                                        <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ $category->name }} Management</span>
                                    </div>
                ...
                <div class="flex items-center gap-6">
                    <div class="hidden md:flex items-center gap-4">
                        <div class="flex items-center gap-3 bg-white/5 hover:bg-white/10 px-5 py-2 rounded-2xl border border-white/10 transition-all group cursor-pointer">
                            <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-white shadow-lg shadow-blue-500/20 group-hover:scale-105 transition-transform">
                                <i class="fas fa-user-shield"></i>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-white text-sm font-bold leading-tight">{{ auth('admin')->user()->name }}</span>
                                <span class="text-blue-400 text-[10px] font-bold uppercase tracking-widest">Administrator</span>
                            </div>
                        </div>
                        
                        <form action="{{ route('admin.logout') }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="w-12 h-12 rounded-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-red-500/20 hover:text-red-400 transition-all border border-transparent hover:border-red-500/30" title="Secure Logout">
                    ...
                    <!-- Mobile Menu Button -->
                    <div class="md:hidden">
                        <button @click="mobileMenu = !mobileMenu" class="w-12 h-12 rounded-xl bg-white/5 text-gray-300 hover:text-white flex items-center justify-center transition-all">
        ...
        <div x-show="mobileMenu" 
            ...
            <div class="px-6 py-6 space-y-4">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-4 px-5 py-4 text-white font-bold rounded-2xl transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-blue-600 shadow-lg shadow-blue-500/20' : 'bg-white/5' }}">
                    <i class="fas fa-th-large"></i> Dashboard
                </a>

                @foreach($permittedMenu as $category)
                    <div x-data="{ open: false }">
                        <button @click="open = !open" class="w-full flex items-center justify-between px-5 py-4 text-gray-300 font-bold rounded-2xl bg-white/5 transition-all">
                        ...
                        <div x-show="open" class="mt-2 ml-4 space-y-2 border-l-2 border-white/10 pl-4 py-2">
                            @foreach($category->modules as $module)
                                <a href="{{ url($module->route) }}" class="block py-3 text-sm font-semibold text-gray-400 hover:text-white transition-colors">
                                    {{ $module->module_name }}
                                </a>
                            @endforeach
                        </div>
                ...
                <div class="pt-6 mt-6 border-t border-white/10">
                    <div class="flex items-center gap-4 px-5 py-4 bg-white/5 rounded-2xl">
                        <div class="w-12 h-12 rounded-xl bg-blue-600 flex items-center justify-center text-white font-bold">
                    ...
                    <form action="{{ route('admin.logout') }}" method="POST" class="mt-4">
                        @csrf
                        <button type="submit" class="w-full h-14 rounded-2xl bg-red-500/10 text-red-400 font-bold flex items-center justify-center gap-3 hover:bg-red-500 hover:text-white transition-all">
