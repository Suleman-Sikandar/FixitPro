...
@section('content')
<form action="{{ route('admin.acl.modules.update', $module->ID) }}" method="POST" class="p-8 space-y-8">
    @csrf
    
    <!-- Hero Section: Module Adjustment -->
    <div class="p-6 bg-blue-50/50 rounded-3xl border border-blue-100/50 space-y-6 relative overflow-hidden group">
        <div class="absolute right-0 top-0 opacity-[0.03] -translate-y-4 translate-x-4">
            <i class="fas fa-sliders-h text-[120px] transition-transform duration-700 group-hover:rotate-12"></i>
        </div>
        ...
        <div class="flex items-center gap-4 text-blue-700 relative z-10">
            ...
            <div>
                <h3 class="text-sm font-black uppercase tracking-[0.2em] leading-none mb-1">Credential Adjustment</h3>
                <p class="text-[10px] text-blue-400 font-bold uppercase tracking-widest">Synchronize system definitions</p>
            </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
            <div class="space-y-2">
                <label class="block text-[10px] font-black text-blue-900/40 uppercase tracking-widest ml-1">Rename Module</label>
                <div class="relative group/input">
                    ...
                    <input type="text" name="module_name" value="{{ $module->module_name }}" required
                           class="w-full pl-11 pr-5 py-3.5 bg-white border border-blue-100 rounded-2xl focus:ring-8 focus:ring-blue-500/5 focus:border-blue-500 transition-all font-bold text-gray-700 shadow-sm">
                </div>
            </div>
            <div class="space-y-2">
                <label class="block text-[10px] font-black text-blue-900/40 uppercase tracking-widest ml-1">Current Slug</label>
                <div class="relative group/input">
                    <span class="absolute left-4 top-1/2 -translate-y-1/2 text-blue-300 group-focus-within/input:text-blue-600 transition-colors">
                        <i class="fas fa-code text-sm"></i>
                    </span>
                    <input type="text" name="slug" value="{{ $module->slug }}" required
                           class="w-full pl-11 pr-5 py-3.5 bg-white border border-blue-100 rounded-2xl focus:ring-8 focus:ring-blue-500/5 focus:border-blue-500 transition-all font-bold text-gray-700 shadow-sm">
                </div>
            </div>
        </div>
        ...
        <div class="space-y-2 relative z-10">
            ...
            <div class="relative group/input">
                ...
                <input type="text" name="route" value="{{ $module->route }}" required
                       class="w-full pl-11 pr-5 py-3.5 bg-white border border-blue-100 rounded-2xl focus:ring-8 focus:ring-blue-500/5 focus:border-blue-500 transition-all font-bold text-gray-700 shadow-sm">
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div class="space-y-2">
            <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Access Group</label>
            <div class="relative group">
                ...
                <select name="category_id" required
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->ID }}" {{ $module->category_id == $category->ID ? 'selected' : '' }}>{{ $category->name }}</option>
                    @endforeach
                </select>
        ...
        <div class="space-y-2">
            ...
            <div class="relative group">
                ...
                <input type="number" name="display_order" value="{{ $module->display_order }}"
    ...
    <div class="flex items-center justify-between p-6 bg-gray-50/30 rounded-3xl border border-gray-100 group transition-all hover:bg-white hover:shadow-xl hover:shadow-gray-200/50">
        <div class="flex items-center gap-4">
            <div class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="show_in_menu" value="1" {{ $module->show_in_menu ? 'checked' : '' }} class="sr-only peer">
            ...
            <div>
                <label class="block text-sm font-bold text-gray-700 leading-none mb-1">Interface Visibility</label>
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Status: <span class="{{ $module->show_in_menu ? 'text-emerald-500' : 'text-gray-400' }}">{{ $module->show_in_menu ? 'Visible in Sidebar' : 'Hidden in Sidebar' }}</span></p>
            </div>
        </div>
    </div>
    ...
    <div class="pt-8 flex flex-col sm:flex-row items-center justify-end gap-4 border-t border-gray-100">
        ...
        <button type="submit" 
            ...
            <i class="fas fa-sync-alt text-xs"></i> Synchronize Core
        </button>
    </div>
</form>
@endsection
