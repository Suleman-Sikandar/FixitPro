...
@section('content')
<form action="{{ route('admin.acl.categories.update', $category->ID) }}" method="POST" class="p-8 space-y-8">
    @csrf
    
    <!-- Hero Section: Category Revision -->
    <div class="p-6 bg-emerald-50/50 rounded-3xl border border-emerald-100/50 space-y-6 relative overflow-hidden group">
        <div class="absolute right-0 top-0 opacity-[0.03] -translate-y-4 translate-x-4">
            <i class="fas fa-folder-open text-[120px] transition-transform duration-700 group-hover:rotate-12"></i>
        </div>
        ...
        <div class="flex items-center gap-4 text-emerald-700 relative z-10">
            ...
            <div>
                <h3 class="text-sm font-black uppercase tracking-[0.2em] leading-none mb-1">Category Revision</h3>
                <p class="text-[10px] text-emerald-400 font-bold uppercase tracking-widest">Synchronize group properties</p>
            </div>
        </div>
        ...
        <div class="space-y-2 relative z-10">
            ...
            <div class="relative group/input">
                ...
                <input type="text" name="name" value="{{ $category->name }}" required
                       class="w-full pl-11 pr-5 py-4 bg-white border border-emerald-100 rounded-2xl focus:ring-8 focus:ring-emerald-500/5 focus:border-emerald-500 transition-all font-bold text-gray-700 shadow-sm">
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div class="space-y-2">
            <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Sequence Rank</label>
        ...
        <div class="space-y-2">
            <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Current Visibility</label>
            <div class="relative group">
                ...
                <select name="active_status" 
                    ...
                    <option value="1" {{ $category->active_status ? 'selected' : '' }}>Publicly Visible</option>
                    <option value="0" {{ !$category->active_status ? 'selected' : '' }}>Globally Hidden</option>
                </select>
    ...
    <div class="pt-8 flex flex-col sm:flex-row items-center justify-end gap-4 border-t border-gray-100">
        ...
        <button type="submit" 
            ...
            <i class="fas fa-sync text-xs"></i> Update Core
        </button>
    </div>
</form>
@endsection
