    ...
    @if(!request()->ajax())
        <div class="px-8 py-6 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-black text-gray-900 tracking-tight">Modify Administrator</h2>
                    <p class="text-xs text-blue-500 font-bold uppercase tracking-widest mt-0.5">{{ $admin_user->email }}</p>
                </div>
            </div>
        </div>
    @endif

        <form action="{{ route('admin.acl.users.update', $admin_user->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            ...
            <div class="flex flex-col items-center gap-5 py-6 bg-gradient-to-b from-gray-50/50 to-white rounded-3xl border border-gray-100 shadow-inner">
                <div class="relative group">
                    ...
                    <img id="preview" src="{{ $admin_user->profile_image ? asset($admin_user->profile_image) : 'https://ui-avatars.com/api/?name='.urlencode($admin_user->name).'&background=0D8ABC&color=fff&size=200' }}" 
                    ...
                    <label for="profile_image" class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-all duration-300 rounded-[2rem] cursor-pointer backdrop-blur-[2px]">
                        <div class="flex flex-col items-center gap-2">
                            ...
                            <span class="text-white text-[10px] font-bold uppercase tracking-wider">Change Image</span>
                        </div>
                    </label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $admin_user->phone) }}"
                               class="w-full pl-11 pr-5 py-3.5 bg-gray-50/5 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-bold text-gray-700 placeholder:text-gray-300"
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <input type="text" name="address" id="address" value="{{ old('address', $admin_user->address) }}"
                    ...
                    <input type="hidden" name="latitude" id="latitude" value="{{ $admin_user->latitude }}">
                    <input type="hidden" name="longitude" id="longitude" value="{{ $admin_user->longitude }}">
                    <input type="hidden" name="city" id="city" value="{{ $admin_user->city }}">
                    <input type="hidden" name="province" id="province" value="{{ $admin_user->province }}">
                    <input type="hidden" name="country" id="country" value="{{ $admin_user->country }}">
                    <input type="hidden" name="postal_code" id="postal_code" value="{{ $admin_user->postal_code }}">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <textarea name="bio" id="bio" rows="3"
                              class="w-full pl-11 pr-5 py-3.5 bg-gray-50/5 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-bold text-gray-700 placeholder:text-gray-300"
                              placeholder="Short professional summary...">{{ old('bio', $admin_user->bio) }}</textarea>
                </div>
            </div>
            ...
            <div class="p-6 bg-blue-50/50 rounded-3xl border border-blue-100/50 space-y-5 relative overflow-hidden group">
                ...
                <div class="flex items-center gap-3 text-blue-700">
                    ...
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-[0.2em]">Credential Security</p>
                        <p class="text-[10px] text-blue-400 font-bold">Leave empty to keep existing password</p>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                    <div class="space-y-2">
                        <label for="password" class="block text-[10px] font-black text-blue-900/40 uppercase tracking-widest ml-1">New Password</label>
                    ...
                    <div class="space-y-2">
                        <label for="password_confirmation" class="block text-[10px] font-black text-blue-900/40 uppercase tracking-widest ml-1">Confirm New</label>
            ...
            <div class="flex items-center justify-between p-6 bg-gray-50/50 rounded-3xl border border-gray-100 group transition-all hover:bg-white hover:shadow-md">
                <div class="flex items-center gap-4">
                    <div class="relative inline-flex items-center cursor-pointer" x-data="{ active: {{ $admin_user->active_status ? 'true' : 'false' }} }">
                        <input type="checkbox" name="active_status" id="active_status" value="1" {{ $admin_user->active_status ? 'checked' : '' }} class="sr-only peer" @change="active = $el.checked">
                    ...
                    <div>
                        <label for="active_status" class="block text-sm font-bold text-gray-700">Active Status</label>
                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Status: <span :class="active ? 'text-emerald-500' : 'text-red-400'" x-text="active ? 'Authorized' : 'Suspended'"></span></p>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-8 flex flex-col sm:flex-row items-center justify-end gap-4 border-t border-gray-100">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-sync-alt text-xs"></i> Synchronize Profile
                </button>
            </div>
        </form>
...
<script>
    ...
    function initAutocomplete() {
        ...
        autocomplete.addListener('place_changed', function() {
            ...
            console.log("📍 Location Synced (Edit):", {
